...
# pre-tuning content to "<name> (before tuning).appDef.backup" (the .backup
# extension, not .appDef, keeps it from ever showing up as an openable DAB
# project) and never touches that backup again, so there's always a way back
# to where you started even though every Save after that overwrites the live
# file directly.
#
...
<style>
 ...
 dialog#saveConfirmDialog{max-width:380px;border:1px solid #454952;border-radius:6px;
   background:#202226;color:#e8e8e8;padding:16px 18px}
 dialog#saveConfirmDialog::backdrop{background:rgba(0,0,0,.5)}
 dialog#saveConfirmDialog p{margin:0;line-height:1.5}
 dialog#saveConfirmDialog .actions{margin-top:14px;text-align:right}
</style></head><body>
<header>
...
<div id="loaderMsg" class="panel">
  ...
  <p>This tuner only works on DAB projects based on FLEx XHTML exports (a LIFT-based project
    folder will be rejected).</p>
...
<dialog id="saveConfirmDialog">
  <p id="saveConfirmMessage"></p>
  <div class="actions"><button id="saveConfirmOkBtn">OK</button></div>
</dialog>
<script>
...
async function loadFromDirectoryHandle(dirHandle){
  // A lexicon picker left over from a previous, abandoned choice (user picked
  // a folder with >1 .xhtml, then hit Choose again before answering it) is no
  // longer relevant to this new folder -- drop it before scanning.
  lexiconPickerEl.classList.add("hidden");
...
if (window.showDirectoryPicker){
  // The picker itself is native OS chrome -- there's no API to set its title
  // or preload an arbitrary path, so the closest we can do is tell the user
  // where to navigate to via this button's own tooltip.
  chooseFolderBtn.title = "Opens in Documents -- navigate to " +
    "App Builder\\Dictionary Apps\\App Projects\\<Your Project>.";
  chooseFolderBtn.addEventListener("click", async () => {
    let dirHandle;
    try {
      // There's no way to hand the picker an arbitrary absolute starting path
      // (browsers only allow a fixed well-known folder, not an arbitrary one)
      // -- the "id" option is documented to make Chrome remember the last
      // folder browsed to under that id, but that didn't hold up in practice
      // here (still reopens at Documents every time), so don't rely on or
      // advertise it. startIn:"documents" is the closest well-known folder to
      // .../Documents/App Builder/Dictionary Apps/App Projects.
...
document.getElementById("saveConfirmOkBtn").onclick = () => saveConfirmDialog.close();
// Save's "close and reopen DAB" instruction is easy to miss as a transient
// flashStatus toast -- it matters enough (skip it and the tuned entry looks
// unchanged) that it needs an explicit OK click, not a timeout.
function showSaveConfirmModal(msg){
  saveConfirmMessage.textContent = msg;
...
// The name ends in .appDef.backup, not .appDef -- DAB enumerates a folder for
// *.appDef to list projects, so a real ".appDef" extension risks the backup
// itself showing up as an openable (and editable) project.
async function ensureBackup(dirHandle, baseName, currentText){
  const backupName = baseName + " (before tuning).appDef.backup";
  try {
...
saveAppDefBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    showSaveConfirmModal(`Saved directly to "${currentProject.appDefName}" (${rules.length} rules). Pre-tuning content is backed up in "${backupName}". Close and reopen DAB to see the change.`);
  } catch (e){
